...
function M.load_keys(ev)
	if IsModuleAvailable("iron") then
		...
		keymap_buffer(
			...
			"n",
			"<C-CR>",
			"<cmd>lua require('iron.core').send_paragraph()<cr>",
			opts,
			"send [p]paragraph (iron)"
		)
		keymap_buffer(
			ev.buf,
			"v",
		...
		keymap_buffer(
			...
			"n",
			"<localleader>sf",
			"<cmd>lua require('iron.core').send_file()",
			opts,
			"send [f]ile (iron)"
		)
		keymap_buffer(
			...
			"n",
			"<localleader>su",
			"<cmd>lua require('iron.core').send_until_cursor()",
			opts,
			"send [u]ntil cursor (iron)"
		)
		...
		-- -- Build
		--
		keymap_buffer(ev.buf, "n", "<localleader>bl", function()
			require("iron.core").send(nil, { "devtools::load_all()" })
		end, opts, "[l]oad all files")
		keymap_buffer(ev.buf, "n", "<localleader>bi", function()
			require("iron.core").send(
				nil,
				{ 'devtools::install(args = "--preclean --with-keep.source --no-multiarch")' }
			)
		end, opts, "[i]nstall pkg")

		keymap_buffer(ev.buf, "n", "<localleader>bb", function()
			require("iron.core").send(nil, { "devtools::build()" })
		end, opts, "[b]uild pkg (iron)")
		keymap_buffer(ev.buf, "n", "<localleader>bT", function()
			require("iron.core").send(nil, { "devtools::test()" })
		end, opts, "[T]est pkg (iron)")
		keymap_buffer(ev.buf, "n", "<localleader>bt", function()
			require("iron.core").send(
				nil,
				{ string.format('testthat::test_file("tests/testthat/test-%s")', vim.fn.expand("%:t")) }
			)
		end, opts, "[t]est file (iron)")
		...
		-- )
		keymap_buffer(ev.buf, "n", "<localleader>bd", function()
			require("iron.core").send(
				nil,
				{ 'devtools::document(roclets = c("rd", "collate", "namespace", "vignette"))' }
			)
		end, opts, "[d]ocument pkg (iron)")
		--
		-- -- Objects
